...
FPSIMASW <- foreach(
  ...
  for (i in 1:n) {
    for (j in i:n) {
      ...
      svwascores <- srs(t(V[i, ]), t(V[j, ]), distances = 'absolute', levels = 0:3, weights = cincweights)
      sbwascores <- srs(t(B[i, ]), t(B[j, ]), distances = 'absolute', levels = 0:1, weights = cincweights)
      svwsscores <- srs(t(V[i, ]), t(V[j, ]), distances = 'squared', levels = 0:3, weights = cincweights)
      sbwsscores <- srs(t(B[i, ]), t(B[j, ]), distances = 'squared', levels = 0:1, weights = cincweights)
